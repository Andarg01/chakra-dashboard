...
const renderMiniChart = (data: { name: string; value: number }[]) => {
  const percentChange = (data[data.length - 1].value - data[0].value) / data[0].value * 100;
  const minChartData = [{ name: '', value: percentChange }];

  return (
    <LineChart width={50} height={30} data={minChartData}>
      <Line
        type="monotone"
        dataKey="value"
        stroke={percentChange >= 0 ? "#00ff00" : "#ff0000"} // Green for positive, red for negative
...
const AnalyticsCard: React.FC<AnalyticsCardProps> = ({ title, value, data, icon }) => {
  ...
  return (
    ...
    >
      ...
      <Flex flex="1" direction="row" justify="space-between" align="flex-end" pr={4}>
