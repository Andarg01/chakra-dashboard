...
const sampleProducts: Product[] = [
  {
    ...
    shopId: "1",
    status: true,
    imageUrls: ["product1_1.jpg", "product1_2.jpg"],
    brand: "Brand A",
    stockQuantity: 100,
    dimensions: ["10x10x10"],
    weight: ["1kg"],
  ...
  },
  {
    ...
    shopId: "2",
    status: true,
    imageUrls: ["product2_1.jpg", "product2_2.jpg"],
    brand: "Brand B",
    stockQuantity: 50,
    dimensions: ["20x20x20"],
    weight: ["2kg"],
...
const Analytics: React.FC = () => {
  const salesData = sampleOrders.map((order) => ({
    shop: sampleShops.find((shop) => shop.id === order.shopId)?.name || "Unknown",
    revenue: order.totalAmount,
  }));

  const brandDistribution: { [key: string]: string[] } = {};
  sampleShops.forEach((shop) => {
    brandDistribution[shop.name] = sampleProducts
      .filter((product) => product.shopId === shop.id)
      .map((product) => product.brand);
  });

  const brandCounts: { [key: string]: number } = {};
  Object.keys(brandDistribution).forEach((shopName) => {
    const brands = brandDistribution[shopName];
