...
const calculatePercentChange = (data: { name: string; value: number }[]) => {
  if (data.length < 2) {
    return 0; // Handle the case when there are less than two data points
  }
...
const AnalyticsCard: React.FC<AnalyticsCardProps> = ({ title, value, data, icon }) => {
  ...
  const percentChange = calculatePercentChange(data);
  ...
  return (
    <Box
      ...
      boxShadow="xl"
      width="300px"
      height="300px" // Set height to one-third of the original height
      display="flex"
    ...
    >
      ...
      <Text fontSize="3xl" fontWeight="normal" mb={2} pl={8}>
        {value}
      </Text>
      <Text fontSize="md" fontWeight="normal" mb={4} pl={8} color={percentChange >= 0 ? 'green' : 'red'}>
