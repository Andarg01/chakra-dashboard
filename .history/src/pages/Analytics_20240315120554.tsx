...
const SalesRevenueCard: React.FC = () => {
  ...
  const increasingRate = ((salesData[salesData.length - 1].revenue - salesData[0].revenue) / salesData[0].revenue) * 100;
  ...
  return (
    <Box boxShadow="xl" p={4} borderRadius="md" w="50%">
      ...
      <Flex direction="column" mt={4}>
        {salesData.map((data) => (
          <Flex key={data.month} justify="space-between" mb={2}>
            <Text>{data.month}</Text>
            <Box bg="blue.200" w={8} h={`${(data.revenue / 40)}%`} borderRadius="md" />
            <Text ml={2}>{`$${data.revenue}`}</Text>
          </Flex>
        ))}
      </Flex>
      <Text mt={4}>Increasing Rate: {increasingRate.toFixed(2)}%</Text>
    </Box>
  );
};
