...
// import { Box, Text, Flex, useColorModeValue, Center } from '@chakra-ui/react';
...
//         borderRadius="lg"
//         boxShadow="xl"
//         width={{ base: '90%', md: '250px', lg: '250px' }} // Adjusted width for responsiveness
//         maxWidth="250px" // Added max width for larger screens
...
const AnalyticsCard: React.FC<AnalyticsCardProps> = ({ title, value, data, icon }) => {
  ...
  return (
    <Box
      ...
      borderRadius="lg"
      boxShadow="xl"
      width={{ base: "100%", sm: "calc(50% - 2rem)", md: "calc(33.33% - 2rem)", lg: "calc(25% - 2rem)" }} // Responsive width
      height="auto" // Set height to auto to minimize the height
      display="flex"
      flexDirection="column"
      margin="1rem" // Reduce margin to fit more cards inline
      position="relative" // Add position relative for absolute positioning of percentage
    >
      <Flex align="center" mb={2}>
        {React.cloneElement(icon, { color: 'green', size: '1.5rem' })} {/* Set icon color and size */}
        <Text fontSize="lg" fontWeight="bold" ml={2}>
          {title}
        </Text>
      </Flex>
      <Text fontSize="xl" fontWeight="normal" mb={2} pl={4}>
        {value}k
      </Text>
      <Flex flex="1" direction="column" justify="flex-end">
        <LineChart width={200} height={80} data={data}> {/* Adjust width and height of chart */}
          <Line
            type="monotone"
            dataKey="value"
            stroke="#00ff00" // Set stroke to green
  ...
  return (
    <Flex justify="center" align="center" minHeight="100vh">
      <Flex direction="row" align="center" flexWrap="wrap">
        <AnalyticsCard title="Total Shop" value={10} data={shopData} icon={<AiOutlineShop />} />
        <AnalyticsCard title="Total Product" value={50} data={productData} icon={<AiOutlineShopping />} />
        <AnalyticsCard title="Total Order" value={303} data={orderData} icon={<AiOutlineFileText />} />
        <AnalyticsCard title="Review" value={20} data={reviewData} icon={<AiOutlineStar />} />
        <AnalyticsCard title="Category" value={45} data={categoryData} icon={<AiOutlineAppstore />} />
      </Flex>
    </Flex>
  );
};
