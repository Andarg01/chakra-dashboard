...
import { BarChart, Bar, Tooltip, Legend, AreaChart, Area, PieChart, Pie, Cell, LineChart, Line } from "recharts";
...
const Dashboard: React.FC = () => {
  ...
  const calculateChangePercentage = (data: { rating: number }[]): string => {
    const firstRating = data[0].rating;
    const lastRating = data[data.length - 1].rating;
    const change = ((lastRating - firstRating) / firstRating) * 100;
    const isIncreasing = change > 0;
    return `${isIncreasing ? '+' : '-'}${Math.abs(change.toFixed(2))}%`;
  };
    ...
    >
      ...
      >
        ...
        <Box textAlign="left">
          ...
          <Text fontSize="lg">Change Percentage: {calculateChangePercentage(reviewData)}</Text>
          <Box>
            <ReviewPage/>
          </Box>
        </Box>
